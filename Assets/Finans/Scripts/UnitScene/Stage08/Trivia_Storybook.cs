...
[RequireComponent(typeof(StoryBook))]
public class Trivia_Storybook : TriviaQuizBase
{
    // StoryBook _storyBook;
    List<string> quizCount = new List<string>();
    public Dictionary<string, object> trivias;
    public int level;
    private string context = "Trivia_Storybook";
    ...
    IEnumerator LoadQuizJSON(string TriviaUrl)
    {
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            currentQuizData = (Dictionary<string, object>)((Dictionary<string, object>)trivias[buttonName])[IFirestoreEnums.StoryBook.levels.ToString()];
            FilterQuizQuestions();
        }
        // CheckQuizDataAsync();
    ...
    /*  async void CheckQuizDataAsync()
      {
          ...
          else
          {
              ...
              Logger.LogError($"No connection found while creating storybook trivia quiz data", context);
          }

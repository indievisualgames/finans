...
using System;
public enum BookFlipMode
{
    RightToLeft,
    LeftToRight
}
public class StoryBookBase : MonoBehaviour
{
    ...
    [HideInInspector]
    public StoryBookPaper[] bookPapers;
    /// <summary>
    ...
    */
    ...
    public void UpdatePages()
    {
        ...
        //Hide all pages
        for (int i = 0; i < bookPapers.Length; i++)
        {
            StoryBookUtility.HidePage(bookPapers[i].Front);
            bookPapers[i].Front.transform.SetParent(BookPanel.transform);
            StoryBookUtility.HidePage(bookPapers[i].Back);
            bookPapers[i].Back.transform.SetParent(BookPanel.transform);
        }

        if (hasTransparentPages)
        {
            //Show the back page of all previous bookPapers
            for (int i = 0; i <= previousPaper; i++)
            {
                StoryBookUtility.ShowPage(bookPapers[i].Back);
                bookPapers[i].Back.transform.SetParent(BookPanel.transform);
                bookPapers[i].Back.transform.SetSiblingIndex(i);
                StoryBookUtility.CopyTransform(LeftPageTransform.transform, bookPapers[i].Back.transform);
            }

            //Show the front page of all next bookPapers
            for (int i = bookPapers.Length - 1; i >= currentPaper; i--)
            {
                StoryBookUtility.ShowPage(bookPapers[i].Front);
                bookPapers[i].Front.transform.SetSiblingIndex(bookPapers.Length - i + previousPaper);
                StoryBookUtility.CopyTransform(RightPageTransform.transform, bookPapers[i].Front.transform);
            }
        ...
        else
        {
            ...
            if (previousPaper >= 0)
            {
                StoryBookUtility.ShowPage(bookPapers[previousPaper].Back);
                //bookPapers[previousPaper].Back.transform.SetParent(BookPanel.transform);
                //bookPapers[previousPaper].Back.transform.SetSiblingIndex(previousPaper);
                StoryBookUtility.CopyTransform(LeftPageTransform.transform, bookPapers[previousPaper].Back.transform);
            }
            //show front of current page only
            if (currentPaper <= bookPapers.Length - 1)
            {
                StoryBookUtility.ShowPage(bookPapers[currentPaper].Front);
                bookPapers[currentPaper].Front.transform.SetSiblingIndex(bookPapers.Length - currentPaper + previousPaper);
                StoryBookUtility.CopyTransform(RightPageTransform.transform, bookPapers[currentPaper].Front.transform);
        ...
        if (enableShadowEffect)
        {
            ...
            if (previousPaper >= 0)
            {
                ...
                LeftPageShadow.transform.SetParent(bookPapers[previousPaper].Back.transform, true);
            ...
            if (currentPaper < bookPapers.Length)
            {
                //has at least one next page, the right shadow should be active
                RightPageShadow.gameObject.SetActive(true);
                RightPageShadow.transform.SetParent(bookPapers[currentPaper].Front.transform, true);
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        Left = bookPapers[currentPaper - 1].Front.GetComponent<Image>();
        StoryBookUtility.ShowPage(Left.gameObject);
        Left.rectTransform.pivot = new Vector2(0, 0);
        Left.transform.position = RightPageTransform.transform.position;
        Left.transform.localEulerAngles = new Vector3(0, 0, 0);

        Right = bookPapers[currentPaper - 1].Back.GetComponent<Image>();
        StoryBookUtility.ShowPage(Right.gameObject);
    ...
    public void ReleasePage()
    {
        if (pageDragging)
        {
            ...
            if (distanceToRight < distanceToLeft && mode == BookFlipMode.RightToLeft)
                TweenBack();
            else if (distanceToRight > distanceToLeft && mode == BookFlipMode.LeftToRight)
                TweenBack();
            else
                TweenForward();
        }
    ...
    public void UpdateBook()
    {
        ...
        if (mode == BookFlipMode.RightToLeft)
            UpdateBookRTLToPoint(f);
        else
            UpdateBookLTRToPoint(f);
    }
    ...
    public void Flip()
    {
        ...
        if (mode == BookFlipMode.LeftToRight)
            currentPaper -= 1;
...
public static class StoryBookUtility
{
    /// <summary>
